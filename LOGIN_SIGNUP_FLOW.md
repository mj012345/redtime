...
## 2. AuthViewModel.signInWithGoogle()

### 위치

- `lib/view/auth/auth_viewmodel.dart` (124-258줄)

### 주요 동작

1. `_isLoading = true`, `_errorMessage = null`, `_isNewUser = null` 설정
...
4. 에러 처리:
   - `FirebaseAuthException`: Firebase Auth 에러 (네트워크, 계정 비활성화 등)
...
6. **⚠️ Firebase Auth에 로그인** (`signInWithCredential`)
   ...
   - `User` 객체 얻음
7. **Firestore에서 기존 사용자 정보 확인** (`getUserFromFirestore`)
...
2. **Firestore에 사용자 정보 저장**:
   ...
   - `AuthService.saveUserToFirestore()` 호출
3. 에러 처리:
   - `FirebaseException`: Firestore 에러 (네트워크, 권한 등)
   - `PlatformException`: 플랫폼 에러
   - 기타 예외: 일반 에러
4. 성공 시: 달력 화면 (`/calendar`)으로 이동

## 5. AuthService.saveUserToFirestore()
...
- `lib/main.dart` (152-280줄)

### 주요 동작

1. `_validateUser()` 호출:
   - Firebase Auth `currentUser` 확인
   - 사용자 정보 갱신 (`user.reload()`)
   - 토큰 유효성 확인 (`getIdToken(true)`)
   - 유효하면 `_isValidUser = true`
2. `build()`:
   - 검증 중: 로딩 표시
   - 유효한 사용자: `StreamBuilder`로 `authStateChanges` 감지 → 달력 화면
   - 유효하지 않은 사용자: 로그인 화면
...
- **요구사항**: 약관 동의 후에만 Firebase Authentication에 사용자를 생성해야 함

### 현재 플로우에서 Firebase Auth 사용자가 생성되는 곳

1. `AuthService.signInWithGoogle()` (113줄)
   ```dart
   final userCredential = await _auth!.signInWithCredential(credential);
   ```
   - Google 로그인 시점에 Firebase Auth에 사용자 생성
   - 약관 동의 전에 이미 Firebase Auth에 사용자가 존재

### 데이터 저장 시점

#### Firebase Authentication

- **시점**: Google 로그인 시 (`signInWithCredential`)
- **문제**: 약관 동의 전에 생성됨

#### Firestore (users/{uid})

- **시점**: 약관 동의 후 (`TermsAgreementView._handleAgreement`)
