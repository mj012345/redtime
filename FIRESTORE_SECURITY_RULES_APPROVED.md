# Firestore Security Rules - 승인된 버전

제안하신 규칙을 약간 개선한 최종 버전입니다.
...
1. **신규 생성 명시**: `!exists(...) && request.resource.data.isDeleted == false` 조건 추가
2. **재가입 안전성**: `exists()` 체크 추가하여 문서 존재 여부 확인
3. **탈퇴 처리 안전성**: `exists()` 체크 추가하여 `resource.data` 접근 전 문서 존재 확인
