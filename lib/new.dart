...
class _FigmaCalendarPageState extends State<FigmaCalendarPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: const _BottomNav(current: _NavTab.calendar),
    );
  }
}
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                const SizedBox(height: 1), // 달력 가로선과 날짜 사이 간격
                SizedBox(
...
class _TodayCard extends StatelessWidget {
  ...
  String? _periodBadgeText() {
    ...
    for (final day in fertileAll) {
      ...
      if (inBlock) {
        final dayIndex = d.difference(start).inDays + 1;
        return '가임기 $dayIndex일째'; // 가임기 동안
      }
      // 가임기 이전: 가장 가까운 다음 가임기 시작 기록
      if (nextFertileStart == null && d.isBefore(start)) {
        nextFertileStart = start;
      }
      // 가임기 이후: 마지막 가임기 종료 기록
      if (d.isAfter(end)) {
    ...
    // [가임기 전] 다음 가임기 시작 전일까지: "가임기 n일전"
    if (nextFertileStart != null) {
      // 만약 예정 생리일이 더 가깝다면, 그 전에 처리되므로 여기선 단순 비교
...
// [위젯 클래스] 하단 네비게이션 바
// ============================================================================
enum _NavTab { calendar, report, my }

Route<T> _noTransition<T>(Widget page) {
          ...
          child: Row(
            ...
            children: [
              _NavItem(
                ...
                label: '달력',
                selected: current == _NavTab.calendar,
                onTap: () {
                  if (current != _NavTab.calendar) {
                    Navigator.of(
                      context,
                    ).pushReplacement(_noTransition(const FigmaCalendarPage()));
                  }
                },
              ),
              _NavItem(
                icon: Icons.insights,
                label: '리포트',
                selected: current == _NavTab.report,
                onTap: () {
                  if (current != _NavTab.report) {
                    Navigator.of(
                      context,
                    ).pushReplacement(_noTransition(const _ReportPage()));
                  }
                },
              ),
              _NavItem(
                icon: Icons.person,
                label: 'MY',
                selected: current == _NavTab.my,
                onTap: () {
                  if (current != _NavTab.my) {
                    Navigator.of(
                      context,
                    ).pushReplacement(_noTransition(const _MyPage()));
                  }
                },
              ),
            ],
...
class _ReportPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: const _BottomNav(current: _NavTab.report),
      backgroundColor: Colors.white,
    );
  }
}
...
class _MyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: const _BottomNav(current: _NavTab.my),
      backgroundColor: Colors.white,
    );
  }
}
